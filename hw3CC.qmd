...
```{python}
search = SearchEngine()
for index, row in dfnyc.itertuples():
	if (~np.isnan(row.LATITUDE)) & (~np.isnan(row.LONGITUDE)) & (np.isnan(row[5])) & (row.BOROUGH == 'NaN'):
		...
		dfnyc.loc(index, row[5]) = r[0]
		if 10001 <= row[5] <= 10282:
			dfnyc.loc(index, row.BOROUGH) = "Manhatten"
		elif 10301 <= row[5] <= 10314:
			dfnyc.loc(index, row.BOROUGH) = "Staten Island"
		elif 10451 <= row[5] <= 10475:
			dfnyc.loc(index, row.BOROUGH) = "Bronx"
		elif 11004 <= row[5] <= 11109 or 11351 <= row[5] <= 11697:
			dfnyc.loc(index, row.BOROUGH) = "Queens"
		elif 11201 <= row[5] <= 11256:
			dfnyc.loc(index, row.BOROUGH) = "Brooklyn"
