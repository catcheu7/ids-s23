...
for row in dfnyc.itertuples():
	if (row._10 != 'NaN') & (np.isnan(row.LATITUDE)) & (np.isnan(row.LONGITUDE)):
		print(row)
```
...
for index, row in enumerate(dfnyc.itertuples(),1):
	if (np.isfinite(row.LATITUDE)) & (np.isfinite(row.LONGITUDE)) & (np.isnan(row._4)) & (row.BOROUGH == 'nan'):
		print(index)
		r = search.by_coordinates(row.LATITUDE, row.LONGITUDE)
		dfnyc.at[index, row._4] = r[0]
		if 10001 <= row._4 <= 10282:
			dfnyc.at[index, row.BOROUGH] = "Manhatten"
		elif 10301 <= row._4 <= 10314:
			dfnyc.at[index, row.BOROUGH] = "Staten Island"
		elif 10451 <= row._4 <= 10475:
			dfnyc.at[index, row.BOROUGH] = "Bronx"
		elif 11004 <= row._4 <= 11109 or 11351 <= row[5] <= 11697:
			dfnyc.at[index, row.BOROUGH] = "Queens"
		elif 11201 <= row._4 <= 11256:
			dfnyc.at[index, row.BOROUGH] = "Brooklyn"
