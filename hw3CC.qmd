...
geoloc = Nominatim(user_agent = 'hw3')

for index, row in enumerate(dfnyc.itertuples(),1):
	if (pd.isna(row._10) == False) & (np.isnan(row.LATITUDE)) & (np.isnan(row.LONGITUDE)):
		latcor = geoloc.geocode(str(row._10 + ", New York City, NY"), timeout = 10000)
		if latcor is None:
			continue
		dfnyc.at[index, "LATITUDE"] = latcor.latitude
		dfnyc.at[index, "LONGITUDE"] = latcor.longitude
```
...
for index, row in enumerate(dfnyc.itertuples(),1):
	if (np.isfinite(row.LATITUDE)) & (np.isfinite(row.LONGITUDE)) & (np.isnan(row._4)) & (pd.isna(row.BOROUGH) == True):
		...
		dfnyc.at[index, "ZIP CODE"] = r[0].zipcode

		zip = int(dfnyc.at[index, "ZIP CODE"])
		if 10001 <= zip <= 10282:
			dfnyc.at[index, "BOROUGH"] = "Manhatten"
		elif 10301 <= zip <= 10314:
			dfnyc.at[index, "BOROUGH"] = "Staten Island"
		elif 10451 <= zip <= 10475:
			dfnyc.at[index, "BOROUGH"] = "Bronx"
		elif 11004 <= zip <= 11109 or 11351 <= zip <= 11697:
			dfnyc.at[index, "BOROUGH"] = "Queens"
		elif 11201 <= zip <= 11256:
...
dfnyc.to_csv("clean.csv", index = True)
