...
from smodel import formula.api as form
...
```{python}
lat = dfnycraw["LATITUDE"].dropna().values
long = dfnycraw["LONGITUDE"].dropna().values
...
```{python}
dfnycraw["Injury"] = (dfnycraw["NUMBER OF PERSONS INJURED"] > 0).astype(int)
itable = pd.crosstab(dfnycraw["Injury"], dfnycraw["BOROUGH"])
